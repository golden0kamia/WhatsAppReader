...
def writeMessage(message):
    ...
    if(message[2] == guyR):  # Write message of person 1 with hour in the right part of the screen
        ...
        if(not known):
            Label(bubble, text=message[2], bg='#DCF8C6', fg='gray', font=(
                'Helvetica', '8')).pack(anchor='ne')
        if(message[3] == "<Médias omis>"):
            showMedia(message, bubble)
        else:
            Label(bubble, text=message[3], wraplength=300,
                  anchor='nw', justify='right', bg='#DCF8C6').pack()
        Label(bubble, text=message[1], bg='#DCF8C6', fg='gray', font=(
            'Helvetica', '8')).pack(anchor='ne')
        bubble.pack(pady=3, anchor='ne', padx=20)
    # Write message of people 2 with hour and name in the left part of the screen (whatsapp group is possible)
    else:
        bubble.configure(bg='#ffffff')
        Label(bubble, text=message[2], bg='#ffffff', fg='gray', font=(
            'Helvetica', '8')).pack(anchor='nw')
        if(message[3] == "<Médias omis>"):
            showMedia(message, bubble)
        else:
            Label(bubble, text=message[3], wraplength=300,
                  anchor='nw', justify='left', bg='#ffffff').pack()
        Label(bubble, text=message[1], bg='#ffffff', fg='gray', font=(
            'Helvetica', '8')).pack(anchor='ne')
...
def openFile():
    ...
    global startMsg
    guyR = askstring(
        "WhatsApp name", "Write your WhatsApp name or leave blank if you don't know")
    #print("guy name ;" + guyR + ";")
    if(not guyR == ""):
        known = True
    # print(known)
    ...
    filePath = askopenfilename(
        title="Selectionner le fichier de discussion", filetypes=[('Text files', '.txt')])
...
def displayConv():
    ...
    # while True:
    if(startMsg - 400 >= 0):
        Button(second_Frame, text="Load fewer",
               bg='#E1F3FB', command=loadFewer).pack(pady=5)
    for x in range(startMsg, startMsg+400):
        ...
        if(lastMssg[0] != mssg[0] and mssg[0] != ""):  # Date changed -> write in chat
            #print("--Label date --")
            Label(second_Frame, text=mssg[0], wraplength=300, anchor='nw',
                  justify='center', relief='groove', bg='#E1F3FB', padx=5).pack(anchor='n')

        if(mssg[2] == "whatsapp"):  # Write whatsapp info message
            #print("--Label info --")
            Label(second_Frame, text=mssg[3], wraplength=300, anchor='nw', justify='center',
                  relief='groove', bg='#E1F3FB', padx=5, font=('Helvetica', '8')).pack(anchor='n')
    ...
    if(startMsg + 400 <= maxData):
        Button(second_Frame, text="Load further",
               bg='#E1F3FB', command=loadFurther).pack(pady=5)
